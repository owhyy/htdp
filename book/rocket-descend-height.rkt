...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname rocket-descend-height) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
; between the ground of the canvas and the center of the rocket
...
; LRCD -> LRCD
; stops the program when ROCKET is at ((+ HEIGHT CENTER) 0)

(check-expect (stop? (- CENTER)) #false)
(check-expect (stop? 10) #false)
(check-expect (stop? (+ CENTER HEIGHT)) #true)
(define (stop? y)
  (if (number? y)
      (>= y (+ CENTER HEIGHT))
      #false))

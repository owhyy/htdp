...
(check-expect (contains? "Toronto" (cons "UBC" (cons "McGill" empty))) true)
...
(check-expect (map2 sqrt (list 4 9))    (list 2 3))
...
;; given a predicate pred an a (list n0 n1 ...) produce list 
;; of all elements for which pred is true 
(check-expect (filter2 positive? empty) empty)
(check-expect (filter2 positive? (list 1 -1 -9 10)) (list 1))
(check-expect (filter2 negative? (list 1 -1 -9 10)) (list -1 -9))
